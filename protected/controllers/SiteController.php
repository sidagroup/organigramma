    ...
    */
    ...
    public function draw($objData,$manager,$respMktg,$respRp,$respDid){
        ...
        foreach ($objData as $key => $value) {
                ...
                foreach ($pma->personsMasters as $masterpma) {
                    ...
                    foreach ($value[2] as $personMasterPms) {                        
                        ...
                        if($personMasterPms->MasterID == $masterpma->MasterID){
                              array_push($pmsArray,['id'=>$personMasterPms->person->PersonID,'name'=>$personMasterPms->person->FirstName,'surname'=>$personMasterPms->person->LastName]);
                        }
            ...
            $pmsUnique = array_unique($pmsArray,SORT_REGULAR);
            $strFullName = "";
            foreach($pmsUnique as $pms){
                 $strFullName .= "\n".ucfirst(strtolower($pms['name'])). " " . ucfirst(strtolower($pms['surname'])) ;
            }
            
            $sheet->setCellValueByColumnAndRow( $posXpma , $posYpma + 4 ,  "PMS ANCONA");
            $sheet->setCellValueByColumnAndRow( $posXpma , $posYpma + 5, $strFullName );
